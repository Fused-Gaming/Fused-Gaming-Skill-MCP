export const generateMockupTool = {
    name: "generate-mockup",
    description: "Generate mobile-first ASCII wireframe mockups for rapid UI prototyping",
    inputSchema: {
        type: "object",
        properties: {
            type: {
    ...
    async handler(input) {
        const { type = "mobile", layout = "single-column", components = [] } = input;
        try {
            const mockup = `
╔════════════════════════════════════════╗
║                                        ║
...
//# sourceMappingURL=generate-mockup.js.map