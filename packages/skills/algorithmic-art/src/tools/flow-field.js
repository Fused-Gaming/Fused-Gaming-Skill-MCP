export const createFlowFieldTool = {
    name: "create-flow-field",
    description: "Create a particle flow field visualization with Perlin noise or other algorithms",
    inputSchema: {
        type: "object",
        properties: {
            algorithm: {
    ...
    async handler(input) {
        const { algorithm, particles = 5000, speed = 1, scale = 0.005, trailLength = 50, width = 1024, height = 768, } = input;
        try {
            if (!algorithm) {
