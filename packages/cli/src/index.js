...
if (hideBin(process.argv).length === 0) {
    ...
    yargs(hideBin(process.argv))
        ...
        .command("add <skill>", "Enable a skill", (yargs) => yargs.positional("skill", {
        describe: "Skill name",
        type: "string",
    }), async (argv) => {
        await add(argv.skill);
    })
        .command("panel", "Launch the SyncPulse panel directly", {}, async () => {
        await launchSyncPulsePanel();
    })
        .command("syncpulse", "Alias for launching the SyncPulse panel", {}, async () => {
        await launchSyncPulsePanel();
    })
        .command("remove <skill>", "Disable a skill", (yargs) => yargs.positional("skill", {
        describe: "Skill name",
        type: "string",
    }), async (argv) => {
